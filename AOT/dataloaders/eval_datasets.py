...
class VOS_Postprocess_Test(Dataset):
    def __init__(self, 
                 root_dict, 
                 images, 
                 labels, 
        ...
        # self.image_root = image_root
        # self.label_root = label_root
        self.frames = root_dict['frames']
        self.save_path = root_dict['save_path']
        self.seq_name = self.root_dict['video_name']
        self.label_root = root_dict['label_root']
        self.images = images
        self.labels = labels  # 00000.png
        self.exp_id = root_dict['expressions_id']
        self.key_frames_num = root_dict['key_frames_num']
    ...
    def read_image(self, idx):
        img_name = self.images[idx]
        img_path = os.path.join(self.root_dict['image_root'], img_name)
        img = cv2.imread(img_path)
        img = np.array(img, dtype=np.float32)
        if self.rgb:
            img = img[:, :, [2, 1, 0]]
    ...
    def read_label(self, label_name):
        label_path = os.path.join(self.root_dict['label_root'], label_name)
        label = Image.open(label_path)
        label = np.array(label, dtype=np.uint8)
        if self.single_obj:
            label = (label > 0).astype(np.uint8)
    ...
    def __getitem__(self, idx):
        # self.image_root = os.path.join(images_root, "valid", 'JPEGImages')
        ...
        sample['meta'] = {'seq_name':self.seq_name, 'frame_num':self.num_frame, 'obj_num':obj_num,
                          ...
                          'save_path':self.root_dict['save_path']
                          }
...
class YOUTUBEVOS_Postprocess_Test(object):
    def __init__(self,
                 root='./datasets/YTB',
                 images_root='',
                 transform=None,
        ...
        self.transform = transform
        # self.seq_list_file = os.path.join(self.db_root_dir, 'meta.json')
        self.seq_test_file = os.path.join(images_root, "meta_expressions", "test", "meta_expressions.json")
        self.seq_list_file = os.path.join(images_root, "meta_expressions", "valid", 'meta_expressions.json')


        self.ann_f = json.load(open(self.seq_list_file, "r"))["videos"]
        valid_test_videos = set(self.ann_f.keys())

        test_videos = set(json.load(open(self.seq_test_file, "r"))["videos"].keys())
        if split == 'valid':
            self.seqs = natsorted(list(valid_test_videos - test_videos))
        elif split == 'test':
            self.seqs = natsorted(list(test_videos))
        else:
            raise ValueError('Wrong split type:{}'.foramt(split))

        score_meta = json.load(open(key_frames_id, 'r'))


        root_dict = {}
        num = 0
        for idx in range(len(self.seqs)):
            video_name = self.seqs[idx]
            video_meta = self.ann_f[video_name]
            text_keys = list(video_meta['expressions'].keys())
            for text_key in text_keys:
                # pdb.set_trace()
                root_dict[num] = {
                    'video_name': video_name,
                    'image_root': os.path.join(images_root, split, 'JPEGImages', video_name),
                    'label_root': os.path.join(root, 'Annotations', video_name, text_key),
                    'save_path': os.path.join(self.result_root, video_name, text_key),
                    'expressions_id': text_key,
                    'frames': self.ann_f[video_name]['frames'],
                    # 'key_frames_num': score_meta[video_name][video_meta['expressions'][text_key]['exp']]["index"]
                    'key_frames_num': score_meta[video_name][text_key]['index_info']["class_index"] # mask_class_index | class_index | mask_index
                    # 'key_frames_num': score_meta[video_name][text_key]['clip_info']["index"]
                    # 'key_frames_num': score_meta[video_name][text_key]['mask_info']["mask_class_index"]
    ...
    def __len__(self):
        return len(self.root_dict)

    def __getitem__(self, idx):

        meta_key = self.root_dict[idx]
        video_name = meta_key['video_name']
        text_key = meta_key['expressions_id']
        key_frames_num = meta_key['key_frames_num']

        # obj_names = list(data.keys())
        images = []
        labels = []
        # for obj_n in obj_names:
        images += map(lambda x: x + '.jpg', list(meta_key["frames"]))
        labels.append(meta_key["frames"][key_frames_num] + '.png')

        images = np.sort(np.unique(images))
        labels = np.sort(np.unique(labels))

        if not os.path.isfile(os.path.join(self.result_root, video_name, text_key, labels[0])):
            if not os.path.exists(os.path.join(self.result_root, video_name, text_key)):
                os.makedirs(os.path.join(self.result_root, video_name, text_key))
            shutil.copy(os.path.join(meta_key['label_root'], labels[0]), 
                        os.path.join(self.result_root, video_name, text_key, labels[0]))

        seq_dataset = VOS_Postprocess_Test(meta_key, images, labels, transform=self.transform, rgb=self.rgb)
